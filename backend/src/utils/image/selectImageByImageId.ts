...
export async function selectImageByImageId(imageId: string): Promise<Image|null> {
    try {
        ...
        const sqlQuery: string = "SELECT BIN_TO_UUID(imageId) AS imageId, BIN_TO_UUID (imageUserId) AS imageUserId, , imageCloudinaryId, imageDateCreated, imageUrl FROM user WHERE imageId = UUID_TO_BIN(:imageId)"
